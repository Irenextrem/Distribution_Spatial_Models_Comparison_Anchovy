...
#                   data=inla.stack.data(stk.est0), family="binomial" , #Le introduzco la matriz de estimación y la familia a la que pertenece
#                   control.compute=list(dic=TRUE,cpo=TRUE, waic=TRUE, return.marginals=TRUE), #Criterios de calidad
#                   control.predictor=list(A=inla.stack.A(stk.est0), compute=TRUE,
...
cpo <- mean(unlist(model_sin_es$cpo)) #Valores de la ordenada predictiva condicional (cpo) y valores de la transformada integral de probabilidad (pit) (interesan valores bajos)
wai<- model_sin_es$waic$waic #Valores del criterio de Watanabe-Akaike y el número efectivo de parámetros estimados.
po<- mean(unlist(model_sin_es$po))
dic<-model_sin_es$dic$dic #El criterio de información de desviación y el número efectivo de parámetros

coe_inla_sin_es <- matrix(ncol=4,nrow=1,c(cpo,wai,po,dic))
colnames(coe_inla_sin_es) <- c('cpo','wai','po','dic')

# write.csv(coe_inla_sin_es,'C:/Users/Irene/Source/Repositorios/TFM-Irene-Extremera/GLM INLA/coe_inla_sin_es.csv')

coe_inla <- read.csv('C:/Users/Irene/Source/Repositorios/TFM-Irene-Extremera/GLM INLA/coe_inla_sin_es.csv');coe_inla[,-1]
...
1-inla.pmarginal(0, model_sin_es$marginals.fixed$tempmean) #P(tempmean>0) la probabilidad de que tempmean sea mayor que 0 es muy grande
...
coe_inla <- read.csv('C:/Users/Irene/Source/Repositorios/TFM-Irene-Extremera/GLM INLA/coe_inla_GLM.csv');coe_inla[,-1]
