...
focal_rac_rast <-focal(sp, w=matrix(1,3,3), fun = mean,  na.rm = TRUE)
...
MoranI <- moran.test(residuals(glm_rac), listw=listw_rac, randomisation=FALSE); MoranI #p valor de 0.02 indica que Ho se rechaza,hay autocorrelación espacial
Moran_MC <- moran.mc(residuals(glm_rac), listw=listw_rac, nsim=100); Moran_MC #p valor de 0.03 Ho  se rechaza, lo cual indica que hay autocorrelación espacial
