...
cosites_glm_inla <-read.csv('C:/Users/Irene/Source/Repositorios/TFM-Irene-Extremera/GLM INLA/coeficientes_glm_inla_todos.csv')
coef_glm_inla_media <- read.csv('C:/Users/Irene/Source/Repositorios/TFM-Irene-Extremera/GLM INLA/coeficientes_glm_inla.csv')
...
# Coeficientes CV sp
cosites_glm_inla_sp <- read.csv('C:/Users/Irene/Source/Repositorios/TFM-Irene-Extremera/GLM INLA/coeficientes_glm_inla_todos_sp.csv')
coef_glm_inla_media_sp <- read.csv('C:/Users/Irene/Source/Repositorios/TFM-Irene-Extremera/GLM INLA/coeficientes_glm_inla_sp.csv')
...
# Modelo
formula0 <- resp~ -1 + beta0  + bathy + odismean +tempmean + salinity
...
fddmx_inla <- function(coordenadas, base_datos,MESH,SPDE){
      ...
      A.Est <- inla.spde.make.A(MESH, loc=cbind(pres_train$x, pres_train$y))

      Stk.Est<-inla.stack(data=list(y=pres_train[,1]), #Variable respuesta
                    A=list(A.Est, 1), #Matriz de covarianzas
                    effects=list(spatial=1:SPDE$n.spde, #Los efectos que quiera
                                 data.frame(beta0=1, pres_train[,c(2:5)])), #meto el b0 y todas las covariables
                    tag='est')

      formula <- y ~ -1 + beta0  + bathy + odismean +tempmean + salinity
      ...
      cpo_modelo.model <- mean(unlist(modelo$cpo)) #cpo

      waic_modelo.model <-  modelo$waic$waic #waic

      po_modelo.model <- mean(unlist(modelo$po)) #po

      dic_modelo.model <- model_sin_es$dic$dic #dic

      return(c(cpo_modelo.model,waic_modelo.model,po_modelo.model,dic_modelo.model))
}

cosites_glm_inla <- matrix(ncol=4,nrow=10)

for (i in 1:10) {cosites_glm_inla[i,] <- fddmx_inla(sdmdata[,c(6,7)],sdmdata,mesh,spde)}

# Le doy nombre a las columnas para saber qué coeficientes hay dentro
colnames(cosites_glm_inla) <- c('CPO','WAIC','PO','DIC')
write.csv(cosites_glm_inla,'C:/Users/Irene/Source/Repositorios/TFM-Irene-Extremera/GLM INLA/coeficientes_glm_inla_todos.csv')
coef_glm_inla_media<- apply(cosites_glm_inla, 2, mean);coef_glm_inla_media
write.csv(coef_glm_inla_media,'C:/Users/Irene/Source/Repositorios/TFM-Irene-Extremera/GLM INLA/coeficientes_glm_inla.csv')
...
1-inla.pmarginal(0, model_sin_es$marginals.fixed$tempmean) #P(tempmean>0) la probabilidad de que tempmean sea mayor que 0 es muy muy grande
1-inla.pmarginal(0, model_sin_es$marginals.fixed$salinity) #P(tempmean>0) la probabilidad de que salinity sea mayor que 0 es casi la mismo que no lo sea
...
# Modelo
formula.1 <- resp ~ -1 + beta0  + bathy + odismean +tempmean + salinity+  f(spatial,model=spde)
...
# fddmx_inla_sp <- function(coordenadas, base_datos,MESH,SPDE){
# 
#       group <- kfold(coordenadas, 5) #kfold(1:1592, 5)
# 
#       pres_train <- base_datos[group != 1, ] #Las que no sean 1
...
#                     tag='est')
# 
#       formula <- y ~ -1 + beta0  + bathy + odismean +tempmean + salinity + f(spatial,model=spde)
# 
#       modelo <- inla(formula,
#                   data=inla.stack.data(Stk.Est), family="binomial" ,
#                   control.compute=list(dic=TRUE,cpo=TRUE, waic=TRUE, return.marginals=TRUE),
...
# }
# 
# cosites_glm_inla_sp <- matrix(ncol=4,nrow=10)
# 
# for (i in 1:10) {cosites_glm_inla_sp[i,] <- fddmx_inla(sdmdata[,c(6,7)],sdmdata,mesh,spde)}
# 
# # Le doy nombre a las columnas para saber qué coeficientes hay dentro
# colnames(cosites_glm_inla_sp) <- c('CPO','WAIC','PO','DIC')
# write.csv(cosites_glm_inla_sp,'C:/Users/Irene/Source/Repositorios/TFM-Irene-Extremera/GLM INLA/coeficientes_glm_inla_todos_sp.csv')
# coef_glm_inla_media_sp <- apply(cosites_glm_inla_sp, 2, mean);coef_glm_inla_media_sp
# write.csv(coef_glm_inla_media_sp,'C:/Users/Irene/Source/Repositorios/TFM-Irene-Extremera/GLM INLA/coeficientes_glm_inla_sp.csv')
...
# 
# 
# saveRDS(model.pred, "C:/User ºs/Irene/Source/Repositorios/TFM-Irene-Extremera/GLM INLA/model_pred.rds")
