...
for (i in 1:10) { cosites_bc.model[i,] <- fddmx1(coords,predictors2,backgr)}
...
# Construyo el modelo
library(rJava)
maxent_model <- maxent(predictors2, data)
plot(maxent_model)
```
Lo que entiendo aquí es que se está midiendo el porcentaje de cada variable en cuando a cómo es de importante a la hora de ajustar el modelo. Según muestra la gráfica las variables mas importantes son la batimetría y la temperatura media.

```{r}

fddmx2 <- function(coordenadas, predictores,background){
      ...
      me_m<- maxent(predictors2, pres_train)
...
# Genereo un bucle for de 10 iteraciones que me devuelva 10 valores diferentes de los 6 coeficientes.
for (i in 1:10) { cosites_maxent_model[i,] <- fddmx2(data,predictors2,backgr)}

# Le doy nombre a las columnas para saber qué coeficientes hay dentro
colnames(cosites_maxent_model) <- c('AUC','COR','Kappa','Sensitivity','Specificity','TSS')

# Hago la media por columnas que es lo que me interesa y lo guardo
coef_bc.model <- apply(cosites_maxent_model, 2, mean)
write.csv(coef_bc.model,'MAXENT/coeficientes_MAXENT.csv')
```
