...
SACAR RESIDUOS -> TRABAJANDO EN ELLO
...
predictors <- stack(files)
names(predictors) <- c("bathy","odismean","salinity","tempmean","focal_rac_vect")

# Escalo los valores para que al hacer el análisis todas las variables tengan la misma escala 

predictors4 <- scale(predictors) #Ya he comprobado que tienen su media en cero en otro script

pbrt_RAC <- predict(predictors4, brt_RAC,type="response", n.trees=brt_RAC$n.trees, shrinkage= 0.01, distribution="bernoulli")
...
rf_rac <- randomForest(model, ddd);rf_rac
```
...
pred_rf_rac <- predict(predictors4, rf_rac,type="response") #Puedo hacer la predicción también
...
La correlación entre lo predicho y observado es del 0.464. Aparte en la gráfica se observa que los valores predichos frente a observados no se ajustan del todo bien a la recta. Lo cual puede indicar...
